...
void spiBegin(uint8_t spi_ch)
{
  ...
  switch(spi_ch)
  {
    case _DEF_SPI1:
      ...
      p_spi->h_spi->PSEL.SCK  = NRF_GPIO_PIN_MAP(0, 14); // SCK
      p_spi->h_spi->PSEL.MOSI = NRF_GPIO_PIN_MAP(0, 13); // MO
      p_spi->h_spi->PSEL.MISO = 0;
      p_spi->h_spi->PSEL.CSN  = NRF_GPIO_PIN_MAP(0,  6); // D11
      p_spi->h_spi->PSELDCX   = NRF_GPIO_PIN_MAP(0,  8); // D12
